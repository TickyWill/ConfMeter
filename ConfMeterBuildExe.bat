:: Creation: A. Chabli 2025-06-30
:: Refactoring: ................

@echo off 
Title ConfMeter.exe making
...
echo:
set /p "cm_branch=Enter ConfMeter branch to import: "
...
:: Setting exe version
echo:
set /p "exe_version=Enter ConfMeter version (#.#.#): "
echo:
...
set "working_dir=%TEMP%\ConfMeter_exe"
:: set "save_dir=C:\Program"
set "save_dir=%USERPROFILE%\ConfMeter_exe"
...
mkdir %working_dir%
if exist %working_dir% (
    echo ******* REPORT ON ConfMeter EXECUTABLE MAKING ******* > %LOG%
...
set "prefixe=https://raw.githubusercontent.com/TickyWill/ConfMeter/"
if %cm_branch%==main (
    set "pgm_origin=%prefixe%%cm_branch%/%app_py%"
) else (
    set "pgm_origin=%prefixe%refs/heads/%cm_branch%/%app_py%"
)
...
:: Renaming the directory dist to aaaa_mm_jj ConfMeter 
...
set fmonth=00%Month%
set fday=00%Day%
set dir_name="%Year%-%fmonth:~-2%-%fday:~-2% ConfMeter"
...
:: Renaming the built exe to ConfMeter-#.#.#
echo Renaming the built exe
set "exe_file_name=ConfMeter-%exe_version%.exe"
